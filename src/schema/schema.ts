 ...
 *
 * Published as a bare `{ type: "object" }` until 0.7.2, so a model filling it in
 ...
 * carried 116pt. 0.8 then gave the whole object ONE unit, the design pixel (see
 * `helpers/design-units.ts`), and this says so.
 *
 * IDENTICAL to the PHP reference's `Schema::styleJsonSchema()` — generated from
 * its export, and `tests/schema-parity.test.ts` diffs the two — so the engines
 * cannot tell a model different things about the same field.
 */
