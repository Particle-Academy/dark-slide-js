 ...
 * in had only the key names, and `fontSize` reads as points. It is design pixels,
 * halved on the way into the file (trap 5 in AGENTS.md): an agent in the
 * fancy-labs document lab set a headline at what it called 232pt and the deck
 * carried 116pt. The units in this object are not all the same either.
 *
 * IDENTICAL to the PHP reference's `Schema::styleJsonSchema()`, and
 * `tests/schema-parity.test.ts` diffs the two exports, so the engines cannot
 * tell a model different things about the same field.
 */
function styleJsonSchema(): Record<string, unknown> {
  return {
    type: "object",
    description:
      "How a text element, or the text inside a shape, looks. The units are NOT uniform: fontSize is in design pixels and is halved into points, while letterSpacing, spaceBefore, spaceAfter, padding, radius and the border and accent-bar widths are already in points, and lineHeight is a multiple.",
    properties: {
